...
function ResourcePage() {
  ...
  const handleCreateFolder = async (e) => {
    ...
    try {
      ...
      setNewFolder("");
      setResourceFolder(createdFolder.name);
    } catch (error) {
  ...
  const handleDeleteFolder = async (folder) => {
    ...
    try {
      ...
      const folderResources = resources.filter(
        (resource) => resource.folder === folderName
      );
      ...
      setResources((prev) =>
        prev.filter((resource) => resource.folder !== folderName)
      );
      ...
      setResourceFolder((current) => {
        ...
        const remainingFolder = folders.find(
          (item) => item.id !== folder.id
        );

        return remainingFolder ? remainingFolder.name : "";
      });
    } catch (error) {
  ...
  const handleAddResource = async (e) => {
    ...
    try {
      ...
      const formattedResource = {
        ...createdResource,
        folder: createdResource.category,
      };
  ...
  const getFolderResources = (folderName) => {
    return resources.filter(
      (resource) => resource.folder === folderName
    );
  };

  const selectedResources = selectedFolder
    ? getFolderResources(selectedFolder)
    : [];
      ...
      <div className="resource-add-section">
        ...
        >
          ...
          <select
            ...
            onChange={(e) => setResourceFolder(e.target.value)}
          >
            <option value="">
              Select Folder
            </option>
        ...
        <section className="resource-folders-section">
          <div className="resource-section-heading">
            <div>
              ...
              <p>
                Click a folder to view its resources.
              </p>
            </div>
          ...
          <div className="resource-folders">
            {folders.map((folder) => {
              const count =
                getFolderResources(folder.name).length;
